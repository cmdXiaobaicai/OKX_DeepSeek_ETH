...
LEVERAGE = 50  # 修改为50倍杠杆
MIN_ORDER_SIZE = 0.02  # 修改最小下单量为0.02ETH
MAX_ORDER_SIZE = 0.1   # 修改最大下单量为0.1ETH
AI_FREQUENCY = 300
...
class DeepSeekAI:
    ...
    def get_trading_decision(self, market_data: Dict, account_status: Dict, position_info: Dict) -> Dict:
        ...
        try:
            ...
            # 优化的系统提示词 - 根据用户要求更新
            system_prompt = """角色定位：你是顶级交易员，专注于OKX交易所的ETH永续合约交易，并且与其他交易员互相竞争
    ...
    def run_dynamic_cycle(self):
        ...
        try:
            ...
            if has_pending:
                ...
                write_echo(f"收集K线数据: 5min×{len(klines_5min)}, 30min×{len(klines_30min)}, 2h×{len(klines_2h)}, 1d×{len(klines_1d)}")
