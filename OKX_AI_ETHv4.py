...
class DeepSeekAI:
    ...
    def get_trading_decision(self, market_data: Dict, account_status: Dict, position_info: Dict, current_tp_sl: Dict = None) -> Dict:
        """获取AI交易决策"""
        try:
            # 在AI请求前记录账户状态和持仓信息
            write_echo("=== AI请求账户状态 ===")
            ...
            if current_tp_sl:
                write_echo(f"当前止盈: {current_tp_sl.get('tp_price', '无')} USDT")
                write_echo(f"当前止损: {current_tp_sl.get('sl_price', '无')} USDT")
            
            # 构建AI提示词 - 更新版模板
            prompt = self._build_prompt(market_data, account_status, position_info, current_tp_sl)
...
- 当前止盈: {current_tp} USDT
- 当前止损: {current_sl} USDT
3. 策略框架
- 多时间维度分析(1m/5m/1h/4h)
- 链上数据与市场情绪结合
- 动态参数调整与风险暴露控制
...
{{
  ...
  "current_position_advice": {{
    "是否保持持仓": "是",                    // 是则继续保持，否则对当前仓位进行平仓
    "是否修改当前止盈止损": "否",             // 是则撤回当前存在的止盈止损单，按照新止盈止损价格重新下单，否则继续保持
    "修改后的止盈价格": 3580.0,               // 建议止盈价格(USDT)
    "修改后的止损价格": 3450.0                // 建议止损价格(USDT)
  }},
  "position_management": {{
    ...
    "take_profit_price": 3580.0              // 建议止盈价格(USDT)
            ...
            formatted_system_prompt = system_prompt.format(
                ...
                used_margin=0,  # 需要从账户信息中获取
                total_equity=account_status["total_equity"],
                margin_ratio=0,  # 需要从账户信息中计算
                position_side=position_info["position_side"],
                position_size=position_info["position_size"],
                entry_price=position_info["entry_price"],
                current_price=market_data["current_price"],
                unrealized_pnl=0,  # 需要计算
                current_tp=current_tp_sl.get('tp_price', '无') if current_tp_sl else '无',
                current_sl=current_tp_sl.get('sl_price', '无') if current_tp_sl else '无'
            )
            ...
            # 记录新增的持仓建议信息
            if 'current_position_advice' in decision:
                write_echo("=== 持仓建议 ===")
                write_echo(f"是否保持持仓: {decision['current_position_advice']['是否保持持仓']}")
                write_echo(f"是否修改止盈止损: {decision['current_position_advice']['是否修改当前止盈止损']}")
                write_echo(f"修改后止盈: {decision['current_position_advice']['修改后的止盈价格']}")
                write_echo(f"修改后止损: {decision['current_position_advice']['修改后的止损价格']}")
    ...
    def _build_prompt(self, market_data: Dict, account_status: Dict, position_info: Dict, current_tp_sl: Dict = None) -> str:
        """构建AI输入提示词 - 更新版模板"""
        input_data = {
            "market_data": {
                "current_price": market_data["current_price"],
        ...
        if current_tp_sl:
            input_data["current_tp_sl"] = current_tp_sl
    ...
    def _parse_ai_response(self, response: str) -> Dict:
        ...
        try:
            ...
            pattern = r'\{\s*"trading_decision"\s*:\s*\{[^{}]*\},\s*"current_position_advice"\s*:\s*\{[^{}]*\},\s*"position_management"\s*:\s*\{[^{}]*\}\s*\}'
    ...
    def _validate_decision_format(self, decision: Dict) -> bool:
        ...
        try:
            ...
            # 检查新增的current_position_advice字段
            if "current_position_advice" in decision:
                cpa = decision["current_position_advice"]
                if not all(field in cpa for field in ["是否保持持仓", "是否修改当前止盈止损", "修改后的止盈价格", "修改后的止损价格"]):
                    return False
...
class OKXTradingExecutor:
    ...
    def execute_trade(self, decision: Dict, current_price: float, is_test: bool = False) -> bool:
        ...
        try:
            ...
            if action == "hold":
                ...
                # 检查是否需要修改止盈止损
                if 'current_position_advice' in decision:
                    self._handle_tp_sl_modification(decision['current_position_advice'])
                return True
                ...
                success = self._close_position(action)
                if success:
                    write_echo("✅ 平仓成功")
                return success
    ...
    def _handle_tp_sl_modification(self, position_advice: Dict):
        """处理止盈止损修改建议"""
        try:
            if position_advice.get("是否修改当前止盈止损") == "是":
                write_echo("需要修改止盈止损")
                # 撤销当前止盈止损单
                self._cancel_current_tp_sl_orders()
                ...
                if position_info["position_size"] > 0:
                    # 重新设置止盈止损
                    new_tp = position_advice.get("修改后的止盈价格", 0)
                    new_sl = position_advice.get("修改后的止损价格", 0)
                    
                    if new_tp > 0 and new_sl > 0:
                        tp_sl_success = self._place_tp_sl_orders_with_retry(
                            position_info["position_side"],
                        ...
                        if tp_sl_success:
                            ...
                            write_error("❌ 止盈止损修改失败")
            else:
                write_echo("无需修改止盈止损")
                
        except Exception as e:
            write_error(f"处理止盈止损修改失败: {e}")
...
class ETHTradingBot:
    ...
    def get_current_tp_sl_info(self) -> Dict:
        """获取当前止盈止损信息"""
        try:
            algo_orders = self.data_collector.get_algo_orders()
            current_tp_sl = {}
            
            for order in algo_orders:
                if 'tpTriggerPx' in order:
                    current_tp_sl['tp_price'] = float(order['tpTriggerPx'])
                if 'slTriggerPx' in order:
                    current_tp_sl['sl_price'] = float(order['slTriggerPx'])
            
            return current_tp_sl
        except Exception as e:
            write_error(f"获取止盈止损信息失败: {e}")
            return {}
    ...
    def run_single_cycle(self):
        ...
        try:
            ...
            # 4. 获取当前止盈止损信息
            current_tp_sl = self.get_current_tp_sl_info()
            
            # 5. AI决策
            ai_decision = self.ai_processor.get_trading_decision(
                market_data, account_status, position_info, current_tp_sl
            )
